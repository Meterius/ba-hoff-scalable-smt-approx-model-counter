...
def deserialize_expression(serialized_expression: str) -> z3.ExprRef:
    return parse_smt2_string(serialized_expression)[0]
...
def random_xor_hash(bits: List[BoolRef]) -> BoolRef:
    ...
    in the smt paper using the `bits` as its parameters
...
def random_xor_hash_equals_zero(bits: List[BoolRef]) -> BoolRef:
    """
    Returns boolean expression of whether the random
    hash value of random_xor_hash equals zero

    Note: Zero means False as random_xor_hash returns a boolean representation
...
def random_m_xor_hash_equals_zero(m: int, bits: List[BoolRef]) -> BoolRef:
    """
    Returns boolean expression of whether the m-times used
    random_xor_hash equals zero

    See random_xor_hash_equals_zero for how it is implemented with m=1

    :param m:
    :param bits:
    """

    return And([random_xor_hash_equals_zero(bits) for i in range(m)])
